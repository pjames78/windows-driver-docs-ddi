...
f1_keywords:
 ...
 - srb/_STORAGE_REQUEST_BLOCK
 - PSTORAGE_REQUEST_BLOCK
 - srb/PSTORAGE_REQUEST_BLOCK
...
# STORAGE_REQUEST_BLOCK structure (srb.h)
...
Specifies the size of the SRB header for compatibility with the **[SCSI_REQUEST_BLOCK](ns-srb-_scsi_request_block.md)** structure. This is equal to the offset of the **Signature** member of this structure.

### -field Function

Set to **SRB_FUNCTION_STORAGE_REQUEST_BLOCK** to indicate that this is an extended SRB. Unlike in **[SCSI_REQUEST_BLOCK](ns-srb-_scsi_request_block.md)**, the SRB function identifier is in the **SrbFunction** member instead.

### -field SrbStatus

Returns the status of the completed request. This member should be set by the miniport driver before it notifies the operating system-specific driver that the request has completed by calling **[ScsiPortNotification](nf-srb-scsiportnotification.md)** with **RequestComplete**. The value of this member can be one of the following:
...
| SRB_FUNCTION_QUIESCE_DEVICE (0x1A) | The request is only between a storage class and storage port driver and is not be sent to miniport. This function serves as a wait by the class driver for port driver to complete all outstanding I/Os. |
...
A guard area to protect against drivers that interpret this structure as **[SCSI_REQUEST_BLOCK](ns-srb-_scsi_request_block.md)**. Set to 0.
...
Points to the Srb extension. A miniport driver must not use this member if it set **SrbExtensionSize** to zero in **[HW_INITIALIZATION_DATA](ns-srb-_hw_initialization_data.md)**. The memory at **MiniportContext** is not initialized by the operating system-specific port driver, and the miniport driver-determined data can be accessed directly by the HBA. The corresponding physical address can be obtained by calling **[ScsiPortGetPhysicalAddress](nf-srb-scsiportgetphysicaladdress.md)** with the **MiniportContext** pointer.
...
Starting in Windows 8, an extended  SRB type is supported with the use of the **STORAGE_REQUEST_BLOCK** structure. **STORAGE_REQUEST_BLOCK** extends SRB functions, allowing extended data blocks for the SRB function to be added to the request. Support for SRB requests using the **[SCSI_REQUEST_BLOCK](ns-srb-_scsi_request_block.md)** structure will continue.

If **NumSrbExData** > 0, the offsets for the SRB extended data blocks are in the  **SrbExDataOffset** array. Each offset is relative to the beginning of this structure and points to a **[SRBEX_DATA](ns-srb-_srbex_data.md)** structure containing the extended data block.
...
- **[SCSI_REQUEST_BLOCK](ns-srb-_scsi_request_block.md)**
